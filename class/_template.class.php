...
class template
{
	...
	function template($db)
	{
		...
		$logo = (isset($this->rowPage->logo))?$this->makeImg("logos/".$this->rowPage->logo):"";
...
?>
	...
	<body>
		<div class="page">
			<table width="100%" class="page" cellspacing=20>
				<tr><td align="center"><?=$logo." ";?></td><td class="bandeau"><h1><?=$this->titre;?></h1></td></tr>
				<tr>
					<td class="menu">
<?
...
*/
	...
	function makeLink($url, $label, $title="", $css="", $options="")
	{
		...
		if (substr(strtolower($url), 0, 5) == "http:")
			return "<a href=\"".$url."\"$title$css$options>$label</a>";
		else
			return "<a href=\"".$GLOBALS["Config"]["URL"]["ROOT"].$url."\"$title$css$options>$label</a>";
	}
...
*/
	 ...
	 * 
	 */
	function ligneListe($colonnes, $actions=null)
	{
		$ret="<tr>\n";
		if (isset($actions))
		{
